...
func (v *VirtletManager) PullImage(ctx context.Context, in *kubeapi.PullImageRequest) (*kubeapi.PullImageResponse, error) {
	...
	imageNameTranslator := v.getImageNameTranslator(in)
...
func (v *VirtletManager) getImageNameTranslator(request *kubeapi.PullImageRequest) imagetranslation.ImageNameTranslator {
	namespace := request.SandboxConfig.Metadata.Namespace
	var sources []imagetranslation.ConfigSource
	if namespace != "kube-system" {
		sources = append(sources, imagetranslation.NewCRDSource(namespace))
	}
