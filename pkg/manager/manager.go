...
func (v *VirtletManager) CreateContainer(ctx context.Context, in *kubeapi.CreateContainerRequest) (*kubeapi.CreateContainerResponse, error) {
	...
	// Was there already started container in this sandbox?
	...
	if remainingContainerId, _ := v.metadataStore.GetPodSandboxContainerID(podSandboxId); remainingContainerId != "" {
		glog.V(3).Infof("CreateContainer: sandbox had already container with id: %s, cleaning it up", remainingContainerId)
		if err := v.libvirtVirtualizationTool.RemoveContainer(remainingContainerId); err != nil {
			glog.Errorf("Error when cleaning up remaining container with id %s: %v", remainingContainerId, err)
			return nil, err
		}
	}
