...
func TestWorkerRunner_ListWorkers(t *testing.T) {
	...
	sandboxId := "sandbox01"
	runner.workers[sandboxId] = make(chan *tail.Line)
	...
	if workers[0] != sandboxId {
		t.Errorf("expected '%s', but obtained '%s'", sandboxId, workers[0])
	}
}
...
func TestWorkerRunner_WorkerExists(t *testing.T) {
	...
	// This is what we're testing here.
	answer := runner.WorkerExists(sandboxId)
...
func TestWorkerRunner_RunNewWorker(t *testing.T) {
	...
	sandboxId := "sandbox01"
	inFile := filepath.Join(baseInDir, sandboxId, "raw.log")
	outFile := filepath.Join(baseOutDir, sandboxId, "_0.log")
...
func TestVirtletLogger_SpawnWorkers(t *testing.T) {
	...
	if workers[0] != "sandbox01" {
		t.Errorf("expected worker 'sandbox01', but got '%s'", workers[0])
	}

	inFile := filepath.Join(baseInDir, "sandbox01", "raw.log")
	outFile := filepath.Join(baseOutDir, "sandbox01", "_0.log")
	expected := fmt.Sprintf("%s,%s", inFile, outFile)
	if logger.workerRunner.(*fakeWorkerRunner).WorkerFiles["sandbox01"] != expected {
...
func TestVirtletLogger_StopObsoleteWorkers_RmOutputDir(t *testing.T) {
	...
	// Expectations.
	if !logger.(*virtletLogger).workerRunner.WorkerExists("sandbox01") {
		t.Errorf("worker 'sandbox01' should still exist")
	}
	if logger.(*virtletLogger).workerRunner.WorkerExists("sandbox02") {
		t.Errorf("worker 'sandbox02' should not exist anymore")
	}
}
...
func setupDirStructure(sandboxIds []string, initialContent string) (string, string, string) {
	...
	for _, sandboxId := range sandboxIds {
		...
		inputFile := filepath.Join(inputDir, "raw.log")
