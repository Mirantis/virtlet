...
func (v *VirtualizationTool) GarbageCollect() (allErrors []error) {
	...
	if errors := v.removeOrphanDomains(ids); errors != nil {
		allErrors = append(allErrors, errors...)
	}

	if errors := v.removeOrphanRootVolumes(ids); errors != nil {
		allErrors = append(allErrors, errors...)
	}

	if errors := v.removeOrphanQcow2Volumes(ids); errors != nil {
		allErrors = append(allErrors, errors...)
	}

	if errors := v.removeOrphanNoCloudImages(ids); errors != nil {
		allErrors = append(allErrors, errors...)
	}
...
func (v *VirtualizationTool) retrieveListOfContainerIDs() ([]string, bool, []error) {
	...
	if err != nil {
		return nil, true, []error{
			fmt.Errorf("can not list pod sandboxes: %v", err),
		}
	}
	...
	for _, sandbox := range sandboxes {
		...
		if err != nil {
			allErrors = append(
				allErrors,
				fmt.Errorf(
					"can not list containers for pod %s: %v",
					sandbox.GetID(),
					err,
				),
			)
...
func (v *VirtualizationTool) removeOrphanDomains(ids []string) []error {
	...
	if err != nil {
		return []error{fmt.Errorf("can not list domains: %v", err)}
	}
	...
	for _, domain := range domains {
		...
		if err != nil {
			allErrors = append(
				allErrors,
				fmt.Errorf("can not retrieve domain name: %v", err),
			)
		}
		...
		if !inList(ids, filter) {
			...
			if err != nil {
				allErrors = append(
					allErrors,
					fmt.Errorf(
						"can not lookup domain '%s' by name: %v",
						name,
						err,
					),
				)
...
func (v *VirtualizationTool) removeOrphanRootVolumes(ids []string) []error {
	...
	if err != nil {
		return []error{fmt.Errorf("can not list libvirt volumes: %v", err)}
	}
	...
	for _, volume := range volumes {
		...
		if err != nil {
			allErrors = append(
				allErrors,
				fmt.Errorf("can not retrieve volume path: %v", err),
			)
			continue
		}
		...
		if !inList(ids, filter) {
			if err := volume.Remove(); err != nil {
				allErrors = append(
					allErrors,
					fmt.Errorf(
						"can not remove volume with path '%s': %v",
						path,
						err,
					),
				)
