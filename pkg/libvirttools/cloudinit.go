...
func (g *CloudInitGenerator) generateUserData() ([]byte, error) {
	...
	writeFilesManipulator.AddConfigMapEntris()
...
func (m *WriteFilesManipulator) AddConfigMapEntris() {
...
func (m *WriteFilesManipulator) addFilesFor(suffix, permissions string) {
	...
	for _, mount := range m.mounts {
		...
		if entries := m.addFilesForMount(mount, permissions); entries != nil {
	...
	scanDirectory := func(name string) error {
		...
		for _, entry := range entries {
			...
			if entry.Mode()&os.ModeSymlink != 0 {
				glog.Infof("Found symlink: %s", entry.Name())
