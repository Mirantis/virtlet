		...
		if err != nil {
			...
			if !haveFsMounts {
				fsMountScript := fmt.Sprintf("if ! mountpoint '%s'; then mkdir -p '%s' && mount -t 9p -o trans=virtio %s '%s'; fi",
					mountPointPathfor9pfs, mountPointPathfor9pfs, mountTagfor9pfs, mountPointPathfor9pfs)
				r = append(r, []interface{}{mountTagfor9pfs, mountPointPathfor9pfs, "9p", "trans=virtio"})
				mountScriptLines = append(mountScriptLines, fsMountScript)
				haveFsMounts = true
			}
		}
...
func generateFsBasedVolumeMounts(mount types.VMMount) ([]interface{}, string, error) {
	...
	mountScriptLine := fmt.Sprintf("if ! mountpoint '%s'; then mkdir -p '%s' && mount --bind %s '%s'; fi",
		mount.ContainerPath, mount.ContainerPath, sourcePath, mount.ContainerPath)

	return []interface{}{mountTagfor9pfs, mount.ContainerPath}, mountScriptLine, nil
}
