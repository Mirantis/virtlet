...
func (v *VirtualizationTool) StopContainer(containerId string) error {
	...
	// Wait until domain is capable of processing shutdown signal or timeout after 60 sec.
	// Repeat sending of shutdown request several times with 10 sec interval
	// To process cases when VM is booting and is not in the state to process shutdown signal
