...
func NewVirtualizationTool(conn *libvirt.Connect, poolName, rawDevices string, metadataStore metadata.MetadataStore) (*VirtualizationTool, error) {
...
func (v *VirtualizationTool) CreateContainer(in *kubeapi.CreateContainerRequest, imageFilepath string, imageSize uint64, netNSPath, cniConfig string) (string, error) {
	...
	v.metadataStore.SetContainer(name, uuid, in.PodSandboxId, config.Image.Image, snapshotName, config.Labels, config.Annotations)
	...
	domXML := generateDomXML(canUseKvm(), name, memory, memoryUnit, uuid, annotations.VCPUCount, cpuShares, cpuPeriod, cpuQuota, snapshotImage, netNSPath, cniConfig)
...
func (v *VirtualizationTool) RemoveContainer(containerId string) error {
	...
	if err := v.RemoveVolume(containerInfo.RootImageSnapshotName); err != nil {
