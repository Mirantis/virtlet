...
func (vol *VirtletVolume) validate() error {
	...
	case "qcow2":
		if vol.Path != "" {
			return fmt.Errorf("qcow2 volume should not have Path but it have it set to: %s", vol.Path)
		}
	case "raw":
		if vol.Capacity != 0 {
			return fmt.Errorf("raw volume should not have set Capacity, but it have it equal to: %d", vol.Capacity)
		}
		if vol.CapacityUnit != "" {
			return fmt.Errorf("raw volume should not have CapacityUnit, but it have it set to: %s", vol.CapacityUnit)
		}
