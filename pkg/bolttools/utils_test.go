...
func SetUpBolt(t *testing.T, sandboxConfigs []*kubeapi.PodSandboxConfig, containerConfigs []*criapi.ContainerTestConfig) *BoltClient {
	...
	for _, container := range containerConfigs {
		if err := b.SetContainer(container.Name, container.ContainerId, container.SandboxId, container.Image, container.RootImageVolumeName, container.Labels, container.Annotations, 0, clock); err != nil {
			t.Fatal(err)
		}
	}
