...
func TestRemoveContainer(t *testing.T) {
	...
	for _, container := range containers {
		...
		if len(contList) != 1 {
			t.Errorf("Expected to have one element (containerID: '%s') in ContainersIDList: '%v' of PodSandbox:'%s'", container.ContainerId, contList, container.SandboxId)
		} else if contList[0] != container.ContainerId {
			t.Errorf("Expected to get containerID: '%s' in ContainersIDList: '%v' of PodSandbox:'%s'", container.ContainerId, contList, container.SandboxId)
		}
		...
		if len(contList) != 0 {
			t.Errorf("Expected to have empty list in ContainersIDList of PodSandbox after removing of container with id: '%s' but have:'%v'", container.ContainerId, contList)
		}
	}
}
