...
func TestSetPodSandbox(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if err := b.SetPodSandbox(tc.config, make(map[string][]byte)); err != nil {
			if tc.error {
				continue
			}
...
func TestRemovePodSandbox(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if tc.sandbox != nil {
			if err := b.SetPodSandbox(tc.sandbox, make(map[string][]byte)); err != nil {
				t.Fatal(err)
			}
		}
...
func TestListPodSandbox(t *testing.T) {
	...
	for _, tc := range tests {
		...
		for _, config := range tc.configs {
			if err := b.SetPodSandbox(config, make(map[string][]byte)); err != nil {
				t.Fatal(err)
			}
		}
