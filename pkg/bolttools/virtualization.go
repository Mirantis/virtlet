...
func (b *BoltClient) SetContainer(containerId, sandboxId, image string, labels, annotations map[string]string) error {
	...
	err = b.db.Update(func(tx *bolt.Tx) error {
		...
		jsonContainerIDArr, err := get(sandboxIDBucket, []byte("ContainersIDList"))
		if err != nil {
			return err
		}

		jsonContainerIDArr, err = AddElToJsonArray(jsonContainerIDArr, containerId)
		if err != nil {
			return err
		}

		if err := sandboxIDBucket.Put([]byte("ContainersIDList"), jsonContainerIDArr); err != nil {
			return err
		}
...
func (b *BoltClient) RemoveContainer(containerId string) error {
	return b.db.Batch(func(tx *bolt.Tx) error {
		...
		jsonContainerIDArr, err = RemoveElFromJsonArray(jsonContainerIDArr, containerId)
		if err != nil {
			return err
		}
