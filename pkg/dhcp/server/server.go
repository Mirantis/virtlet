...
func (s server) NewConfigManipulator(socketPath string) (configManipulator, error) {
	cm := configManipulator{mutex: s.mutex, configuration: s.configuration}
	if err := cm.SetupListener("unix", socketPath); err != nil {
		return configManipulator{}, err
	}
	return cm, nil
}

func (s server) NewDHCPServer(IPToListen string) (dHCPServer, error) {
	dhcp := dHCPServer{mutex: s.mutex, configuration: s.configuration}
	if err := dhcp.SetupListener(IPToListen + ":67"); err != nil {
