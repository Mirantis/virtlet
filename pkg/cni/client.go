...
func NewClient(pluginsDir, configsDir string) (*Client, error) {
	netConfig, err := ReadConfiguration(configsDir)
	glog.V(3).Infof("CNI config: name: %q type: %q", netConfig.Plugins[0].Network.Name, netConfig.Plugins[0].Network.Type)
	if err != nil {
...
func (c *Client) Type() string { return c.netConfig.Plugins[0].Network.Type }
...
func (c *Client) AddSandboxToNetwork(podId, podName, podNs string) (*cnicurrent.Result, error) {
	...
	result, err := c.cniConfig.AddNetworkList(c.netConfig, rtConf)
	if err == nil {
...
func (c *Client) RemoveSandboxFromNetwork(podId, podName, podNs string) error {
	...
	err := c.cniConfig.DelNetworkList(c.netConfig, c.cniRuntimeConf(podId, podName, podNs))
	if err == nil {
