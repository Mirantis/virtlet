...
function setup_minion() {
    lxc-attach -n $1 -- touch ${BASE_IMAGE_WORK_DIR}/start_minion.sh
...
"
    lxc-attach -n $1  --  sudo /bin/bash -c "echo \"$cmd\" > ${BASE_IMAGE_WORK_DIR}/start_minion.sh"
    lxc-attach -n $1  -- sudo chmod +x ${BASE_IMAGE_WORK_DIR}/start_minion.sh
    lxc-attach -n $1  -- sed -i -e '$i'"${BASE_IMAGE_WORK_DIR}/start_minion.sh"'\' /etc/rc.local
    lxc-attach -n $1  -- sudo bash -x ${BASE_IMAGE_WORK_DIR}/start_minion.sh
}

function create-update-base-container() {
    if [ -z "`lxc-ls | grep ${BASE_CONTAINER_NAME}`" ]; then
...
function copy-virtlet-images() {
    if [ -f "./${VIRTLET_IMAGE_NAME}.tar" ]; then
       rm "./${VIRTLET_IMAGE_NAME}.tar"
    fi

    if [ -f "./${LIBVIRT_IMAGE_NAME}.tar" ]; then
       rm "./${LIBVIRT_IMAGE_NAME}.tar"
    fi
...
function cluster_up() {
    ...
    do
       lxc-copy -n ${BASE_CONTAINER_NAME} -N "${CONTAINER_PREFIX_NAME}minion$i"
       lxc-start -n "${CONTAINER_PREFIX_NAME}minion$i"
       lxc-wait --name "${CONTAINER_PREFIX_NAME}minion$i" --state RUNNING
       wait_for_ip "${CONTAINER_PREFIX_NAME}minion$i"
       setup_minion "${CONTAINER_PREFIX_NAME}minion$i"
       lxc-stop -n "${CONTAINER_PREFIX_NAME}minion$i"
       lxc-start -n "${CONTAINER_PREFIX_NAME}minion$i"
    done
