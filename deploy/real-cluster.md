...
For Virtlet and CRI proxy to work, there are the following prerequisites for the
nodes that run Virtlet:
...
The DaemonSet defaults to using CRI Proxy bootstrap, but the mode is
...
(this requires `jq` utility which can be installed with `apt-get install jq` on Ubuntu):
...
```ini
[Service]
Environment="KUBELET_EXTRA_ARGS=--experimental-cri --container-runtime=remote --container-runtime-endpoint=/run/criproxy.sock --image-service-endpoint=/run/criproxy.sock --enable-controller-attach-detach=false"
```
