...
function demo::get-dind-cluster {
  ...
  if [[ -f ${dind_script} ]]; then
    demo::step "Found existent ${dind_script}, it can be not up to date"
    if [[ ! ${NONINTERACTIVE} ]]; then
        demo::ask-user "Do you want to reload ${dind_script} ?" download
        if [[ ${download} = "true" ]]; then
          rm "${dind_script}"
        fi
    else
       demo::step "Will now clear existent ${dind_script}"
       rm "${dind_script}"
    fi
  fi
