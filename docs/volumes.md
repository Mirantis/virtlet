...
## Providing files to VM using Secrets or ConfigMaps

Virtlet supports standard definitions of Secrets and ConfigMaps.
It generates configuration for `write_files`
[cloud-init](http://cloudinit.readthedocs.io/en/latest/index.html)
module basing on populated by kubelet Secret's/ConfigMap specific
directory content.
...
In the same manner as files in ConfigMaps user can use Secret according to
