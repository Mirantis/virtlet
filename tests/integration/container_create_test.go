...
func TestContainerVolumes(t *testing.T) {
	...
	defer ct.teardown()
	ct.sandboxes[0].Annotations["VirtletVolumes"] = `[{"Name": "vol1"}, {"Name": "vol2"}, {"Name": "vol3"}]`
	ct.sandboxes[0].Annotations["VirtletVolumes"] = `[{"Name": "vol1"}, {"Name": "vol2"}]`
	ct.pullAllImages()
	...
	for idx, sandbox := range ct.sandboxes {
		...
		if _, exists := sandbox.Annotations["VirtletVolumes"]; exists {
			exp = fmt.Sprintf("%d", len(strings.Split(sandbox.Annotations["VirtletVolumes"], ",")))
		}
		verifyUsingShell(t, cmd, "attached ephemeral volumes", exp)
	}
