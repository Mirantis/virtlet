...
func checkFilesystemAccess(ssh framework.Executor) {
	...
	out, err := framework.ExecSimple(ssh, "ls -l /mnt")
	Expect(err).NotTo(HaveOccurred())
...
func setupCeph() (string, string) {
	...
	executor := container.Executor(false, "")
	By("Waiting for ceph cluster")
	Eventually(func() error {
		_, err := framework.ExecSimple(executor, "ceph", "-s")
		return err
	}).Should(Succeed())
	...
	for _, cmd := range commands {
		out, err = framework.ExecSimple(executor, "/bin/bash", "-c", cmd)
		Expect(err).NotTo(HaveOccurred())
	}
	return monIP, out
}
