...
var _ = Describe("Basic cirros tests", func() {
	...
	Context("Virtlet logs", func() {
		...
		BeforeAll(func() {
			...
			Expect(err).NotTo(HaveOccurred())
			executor, err = virtletPod.DinDNodeExecutor()
			Expect(err).NotTo(HaveOccurred())
		...
		It("Should contain login string in VM log", func() {
			Expect(framework.ExecSimple(executor, "/bin/bash", "-c",
				fmt.Sprintf(
		...
		It("Should contain login string in pod log and each line of that log must be a valid JSON", func() {
			out, err := framework.ExecSimple(executor, "/bin/bash", "-c",
				fmt.Sprintf("cat '/var/log/pods/%s/%s'", sandboxID, filename))
			Expect(err).NotTo(HaveOccurred())
			found := 0
	...
	It("Should provide VNC interface", func(done Done) {
		...
		executor, err := pod.Container("virtlet")
		Expect(err).NotTo(HaveOccurred())
		...
		do(framework.ExecSimple(executor, "vncsnapshot", "-allowblank", display, "/vm.jpg"))
	}, 60)
})
