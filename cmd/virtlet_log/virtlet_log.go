...
* besides Virtlet container. It periodaically checks for new log directories and spawns
* one worker for each log directory. Worker then "tail -F"s log file and converts each line
...
`, virtletDir, kubernetesDir, sleepInterval)
	...
	for {
		...
		glog.V(1).Infof("Sleep %d seconds", sleepInterval)
