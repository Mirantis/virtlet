	...
	for {
		...
		case <-exitEOF:
			...
			if err := cmd.Process.Signal(syscall.SIGKILL); err != nil {
				glog.Errorf("Error on forwarding SIGKILL to child %v!", err)
			}
			procTerminated = true
		case <-sigTERM:
			glog.Infof("Received SIGTERM. Forwarding to child and wait the process to stop.!")
			if err := cmd.Process.Signal(syscall.SIGTERM); err != nil {
				glog.Errorf("Error on forwarding SIGTERM to child %v!", err)
			}
			procTerminated = true
		}
	}
...
func catchParentKilled(exitEOF chan bool) {
	// Child peocess  has been spawned with setting
...
func parent(exitEOF chan bool, sigTERM chan os.Signal) {
	childArgs := append([]string{"-child"}, os.Args[1:]...)
...
func child(exitEOF chan bool, sigTERM chan os.Signal) {
	...
	if emulator == "" || nsPath == "" {
		...
		// here
		if err, _ := runCommand(defaultEmulator, emulatorArgs, exitEOF, sigTERM); err != nil {
